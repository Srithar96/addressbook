    ...
    {   
       try
       {
          conn = DBConnection.getInstance().getConnect();
           ...
           String qry;
           qry = "insert into AddressBook values ('"+person.getName()+"','"+person.getMobile()+
                   "','"+person.getEmail()+"','"+person.getCollege()+"')";
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update AddressBook set name='"+person.getName()+"',mobile ='"+
                    person.getMobile()+"',email='"+person.getEmail()+"',college='"+person.getCollege()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
